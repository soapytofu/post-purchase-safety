...
export default async function Dashboard() {
  ...
  return <div className="page">
    ...
    <section className="trust-strip"><CheckCircle2 size={18} /><span><strong>Demo data is clearly labeled.</strong> Matches are inferences, not safety determinations. Always verify the issuing authority.</span><SyncButton /></section>
    <section className="metrics">
      ...
      <article><span className="metric-icon blue"><Database size={20} /></span><p>Most recent sync</p><strong className="metric-date">{sync ? date(sync.syncedAt) : "Not yet"}</strong><span className="metric-caption">{sync?.recordCount ?? 0} demo notices</span></article>
    </section>
    ...
    <footer className="sync-foot"><Clock3 size={14} /> Last checked {sync ? date(sync.syncedAt) : "never"} · Local fixture provider</footer>
  </div>;
}
